...
2. Если информация есть в видео — указывать название урока и тайм-код
3. Отвечать кратко и по делу
4. Если в контексте нет ответа — честно сказать об этом

Формат ответа:
- Краткий ответ на вопрос
- Ссылка на видео: "📹 Подробнее: {название урока} ({тайм-код})"
    ...
    def format_context(self, chunks: list[dict]) -> str:
        ...
        for i, chunk in enumerate(chunks, 1):
            context_parts.append(
                f"[Источник {i}]\n"
                f"Модуль: {chunk.get('module_title', 'Неизвестно')}\n"
                f"Урок: {chunk.get('lesson_title', 'Неизвестно')}\n"
                f"Тайм-код: {chunk.get('timestamp', '00:00')}\n"
